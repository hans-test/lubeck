...
-- visualizeTest :: (Show s, D.HasColors a, D.HasLines a, D.HasText a) => [s] -> Geometry SVG -> [Aesthetic s] -> Draft a
visualizeTest dat geom aess = flip getStyled mempty
  $ drawPlot
  $ plot dat aess geom
...
test2 = DrawingTest "test2" ""
  $ visualizeTest ([(1,2), (3,4)] :: [(Int, Int)]) line
  [ mempty
  , x <~ _1
  , y <~ to snd
  ]
test3 = DrawingTest "test3" ""
...
test20 = DrawingTest "test20" ""
  $ flip getStyled mempty $ drawPlot $
...
{-
  Same type/scale.
-}
test24 = DrawingTest "test24" ""
  $ flip getStyled mempty $ drawPlot $ mconcat $ zipWith putTogether geoms dat
  where
...
{-
  Bar plot (horizontal).
-}
test26 = DrawingTest "test26" ""
  $ flip getStyled (barPlotOrientation .~ Horizontal $ mempty) $ drawPlot $
  plot (zip chars freq) [x <~ _1, y <~ _2] bars
  where
    chars :: [Char]
    freq :: [Int]
...
-}
test27 = DrawingTest "test27" ""
  $ flip getStyled (barPlotOrientation .~ Vertical $ hoverSelectStates .~ hss $ mempty) $ drawPlot $
...
Bar plot with highlights (horizontal):
  ...
  - 3rd and 5th element selected
-}
test28 = DrawingTest "test28" ""
  $ flip getStyled (barPlotOrientation .~ Horizontal $ hoverSelectStates .~ hss $ mempty)
  $ drawPlot
  $ plot (zip chars freq) [x <~ _1, y <~ _2 `withScale` linearWithOptions IntegerLR UseZero] bars
  where
    chars :: [Char]
...
{-
  Pie/circular plot.
-}
test29a = DrawingTest "test29a" "" $
  flip getStyled (noXY .~ Any True $ mempty) $ drawPlot $
...
test30 = DrawingTest "test30" "" $ flip getStyled mempty $ drawPlot $ mconcat
...
test31 = DrawingTest "test31" "" $ flip getStyled mempty $ drawPlot $ mconcat
...
test32 = DrawingTest "test32" "" $ flip getStyled mempty $ drawPlot $ mconcat
...
-- TODO
test33 = DrawingTest "test33" "" $ flip getStyled mempty $ drawPlot $ mconcat
...
test34 = DrawingTest
  ...
  |]
  $ flip getStyled mempty $ drawPlot $ plot persons [x <~ to height33,
                             y <~ to weight,
...
-}
test35 = DrawingTest "test35" "" $ flip getStyled mempty $ drawPlot $
  plot (zip
    ([1..10] :: [Int])
    ([0,1,5,5,4,3,4,4,4,3] :: [Int])
    ) [x<~_1,y<~_2]
